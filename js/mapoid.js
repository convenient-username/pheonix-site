	...
	mapoidPlugin.prototype = 
	{
		...
		_redrawCoords : function()
		{
			...
			self.map.find('area').each(function()
			{
				...
				var coords = $this.data(c).split(/[, ]/), // Split on spaces or commas, not just commas. Would return NaN's before.
		 ...
		 */	
			...
			var	$canvas = canvas[0],
				...
				//$canvas.height = h;
				var cox,coy
				var	coords = area.attr('coords').split(/[, ]/), // Split on spaces OR commas, would return NaN's before.
					cox = [];
					coy = [];
		...
		/* Shape coordinate apportioning */
			...
			else
			{
				...
				else
				{
					...
					if(shapeSort == 'polygon' || shapeSort == 'poly' )  // CHANGED
					{
